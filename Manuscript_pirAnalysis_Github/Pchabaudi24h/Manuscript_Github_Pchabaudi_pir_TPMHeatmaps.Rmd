...
#Add chapl_aapl numbers for easier heatmap drawing later.
cir_info <- cir_info %>% 
  group_by(chapl_aapl_name) %>% 
  mutate(chapl_aapl_name_num = n_distinct(Geneid)) %>% 
  mutate(chapl_aapl_name_num = paste0(
                       chapl_aapl_name, 
                       ' (n = ',
                       chapl_aapl_name_num,
                       ')'
...
annotation_col <- c(chapl_aapl_name_col_num, 'ref' = 'black')
...
chapl_aapl_name_lgd <- Legend(
  title = 'sub-family',
  at = factor(labels_chapl_aapl_name,
