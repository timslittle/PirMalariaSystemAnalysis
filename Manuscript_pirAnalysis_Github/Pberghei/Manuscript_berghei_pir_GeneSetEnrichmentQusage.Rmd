...
Initially I used counts here but I don't know if I made that up or not... Going to try with log(TPM+1).
...
names(chrom_list) <- paste0('chr', 
                            unique(pir_info$chrom))
...
names(maleGam_v_Asex.Mix.results) <- c('all', 'subfam', 'SorL', 'chrom')
...
plotGeneSetDistributions(maleGam_v_Asex.Mix.results$chrom$results, path.index = 'chr14')
```
...
names(Asex.Mix_v_Liv.Meroz.results) <- c('all', 'subfam', 'SorL', 'chrom')
...
```{r}
berghei_comparison_stats_df
```


```{r binom_glm}
berghei_tpm <- read_xlsx('Manuscript_Frontiers_SuppInfo_berghei.xlsx',
                      sheet = 2) %>% 
  melt(value.name = 'tpm', 
       variable.name = 'stage') %>% 
  mutate(experiment = as.factor(
    str_extract(stage, 
                pattern = '^Expt[[:digit:]]{2}')
    ),
stage = as.factor(
  str_extract(stage, 
              pattern = '(?<=Expt[[:digit:]]{2}_).+(?=_[[:digit:]]{1,2})')
  ),
Geneid = as.factor(Geneid),
tpm = as.factor(tpm)
)
