...
tpm = function(count_data,lengths) {
  ...
  if(any(grepl(colnames(lengths), 
    ...
    if(all(count_data[grep(colnames(count_data), 
                           pattern = 'id', ignore.case = T)] == lengths[grep(colnames(lengths), 
    ...
    stop('Count data and/or length table supplied do not have an id column, make sure they are ordered correctly')
  }
  
  #Remove the id column from supplied count_data
  if(any(grepl(colnames(count_data), pattern = 'id',ignore.case = T)) == T){
    counts_tpm = count_data[,-grep(colnames(count_data), pattern = 'id', ignore.case = T)]
  } else{
  ...
  if(any(grepl(colnames(count_data), pattern = 'id', ignore.case = T)) == T){
    counts_tpm = bind_cols(count_data[,grep(colnames(count_data), pattern = 'id', ignore.case = T), drop = FALSE], 
                           counts_tpm)
  } 
  return(counts_tpm)
}