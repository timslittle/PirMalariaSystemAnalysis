...
berg_descr <- read_csv('../../genomes_genes_annotations/standard_files/PlasmoDb-47_beta_Pberghei_GenesByTaxon_Summary.csv') %>% 
  mutate(Geneid = `Gene ID`,
         Description = `Product Description`) %>% 
  select(c(Geneid, Description))
berg_pir <- berg %>% 
...
```{r yoelii_data}
yeol_path <- list.files('..',
                        pattern = 'Hart_2019_Pyeolii',
                        recursive = TRUE,
                        full.names = TRUE)
yeol_counts_full <- read_xlsx(yeol_path, 
                         sheet = 2) %>% 
...
yeol_counts <- select(yeol_counts_full, -Description)

yeol_lengths_path <- list.files('..',
                                pattern = 'PlasmoDBv46_Pyoelii17X_',
                                recursive = TRUE,
                                full.names = TRUE)
yeol_lengths <- read_csv(yeol_lengths_path) %>% 
  mutate(Geneid = `Gene ID`,
         length = `Transcript Length`) %>% 
  select(c(Geneid, length))
#One gene 'PY17X_0902700' has two different length transcripts - take median
yeol_lengths %>% group_by(Geneid) %>% dplyr::summarise(n = n()) %>% dplyr::arrange(desc(n))

#This gives 5074.5 so need to make sure that not having an integer count messes up things but it shouldn't
yeol_lengths <- yeol_lengths %>% 
  group_by(Geneid) %>% 
  dplyr::summarise(length = median(length))
nrow(yeol_lengths)
nrow(yeol_counts)

yeol_tpm <- tpm(count_data = yeol_counts, lengths = yeol_lengths)
yeol_tpm <- yeol_tpm %>% 
  mutate(Description = yeol_counts_full$Description[match(Geneid, 
                                                          yeol_counts_full$Geneid)]) %>% 
  mutate(ancestral_ortholog = ifelse(Geneid %in% gene_names,1,0))

yeol_tpm_pir <- yeol_tpm %>% 
  filter(str_detect(Description, pattern = 'PIR|YIR') | ancestral_ortholog == 1) %>% 
  filter(!str_detect(Description, pattern = 'pseudo|fragment') | ancestral_ortholog == 1) %>% 
  dplyr::arrange(desc(tpm))

Fn_yeol <- ecdf(yeol_tpm_pir$tpm)

plot(Fn_yeol)

list_yeol_percantile <- lapply(yeol_tpm_pir$Geneid[yeol_tpm_pir$ancestral_ortholog == 1], 
                               function(ancestr){
                                 list(Fn_yeol(filter(yeol_tpm_pir, Geneid %in% ancestr)$tpm))
                               })
yeol_percentile <- data.frame(Geneid = yeol_tpm_pir$Geneid[yeol_tpm_pir$ancestral_ortholog == 1],
                               percentile = unlist(lapply(list_yeol_percantile, `[`, 1))) 
yeol_percentile

yeol_percentile_all <- ecdf_percentile(yeol_tpm)

```

```{r chabaudi_percentile}

chab_info <- read_csv('../../genomes_genes_annotations/standard_files/PlasmoDB-47_beta_Pchabaudichabaudi_GenesByTaxon_Summary.csv') %>% 
  mutate(Geneid = `Gene ID`,
         Description = `Product Description`) %>% 
  select(c(Geneid, Description))
